...
public sealed class JsonToGoConverter(string jsonText, string structName, GoTypes unknownTypeOption, bool enableOptionalField)
{
    ...
    private string RenderRoot(SchemaNode schema)
    {
        ...
        {
            string itemTypeName = $"{_structName}Item";
            string itemType = RenderStructType(schema.ItemSchema.Properties!, 0);
            return
                $"type {_structName} []{itemTypeName}{Environment.NewLine}{Environment.NewLine}type {itemTypeName} {itemType}";
        }

        string goType = RenderGoType(schema, 0);
        return $"type {_structName} {goType}";
    }
    ...
    private string RenderStructType(IReadOnlyDictionary<string, SchemaNode> properties, int indentLevel)
    {
        ...
        int maxFieldTypeLength = fields.Count > 0 ? fields.Max(field => field.FieldType.Length) : 0;
        ...
        foreach ((string fieldName, string fieldType, string jsonPropertyName) in fields)
        {
            ...
            builder.Append(fieldIndent)
                ...
                .Append(' ')
                .Append(fieldType.PadRight(maxFieldTypeLength))
                .Append(' ')
                .Append('`')
                .Append("json:\"")
    ...
    private static string ToGoFieldName(string jsonPropertyName)
    {
        ...
        // {
        //     "Type" or "Map" or "Chan" or "Func" or "Var" or "Range" or "Select" or "Go" or "Defer" or "Fallthrough"
        //         or "Interface" or "Struct"
        //         => fieldName + "Field",
        //     _ => fieldName
        // };
        return fieldName;
    ...
    private sealed class SchemaNode
    {
        ...
        private SchemaNode(SchemaKind kind, IReadOnlyDictionary<string, SchemaNode>? properties = null,
            SchemaNode? itemSchema = null)
        {
            Kind = kind;
        ...
        public static SchemaNode Object(IReadOnlyDictionary<string, SchemaNode> properties) =>
            new(SchemaKind.Object, properties);
